...
last_ten_years = season_by_season_stats[season_by_season_stats['season'].between(start_year,end_year)]

st.write(f'Compare season stats from {start_year}-{end_year} season')
options = st.multiselect(
     "Select three players to compare", last_ten_years['player'].drop_duplicates().tolist(), max_selections=3)

point_totals = last_ten_years[['season', 'player', 'pts']]
